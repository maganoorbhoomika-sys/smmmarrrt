...
# ── Train model on startup (no .pkl files needed) ────────────
@st.cache_resource
def train_model():
    ...
    # Replace biological zeros with median
...
if st.button("Predict", use_container_width=True, type="primary"):
    ...
    if pred == 1:
        st.error(f"⚠️ High risk of Diabetes — predicted probability: **{proba:.1%}**")
    else:
        st.success(f"✅ Low risk of Diabetes — predicted probability: **{proba:.1%}**")
